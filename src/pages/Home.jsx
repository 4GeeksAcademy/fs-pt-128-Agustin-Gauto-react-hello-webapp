			...
			<div className="container">
				{
					store.contacts.map(contact => (
						<div key={contact.id} className="card border-dark m-3 p-3" >
							...
							<p>Email: <i class="fa-solid fa-envelope"></i> {contact.email}</p>
							<p>Phone: <i class="fa-solid fa-phone"></i> {contact.phone}</p>
							<p>Address: <i class="fa-solid fa-location-dot"></i> {contact.address}</p>
							</div>
							</div>
							<div>
								<button className="btn m-0" type="button"> <i class="fa-solid fa-pencil"></i> </button>
								<button className="btn m-0" type="button"> <i class="fa-regular fa-trash-can"></i> </button>
							</div>
							</div>
						</div>
					))
